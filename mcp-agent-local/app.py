...
# 2. Clé API Mistral (définie dans l'environnement ou saisie par l'utilisateur)
...
if user_prompt := st.chat_input("Ex: Quel temps fait-il à Rome ?"):
    if not MISTRAL_API_KEY:
        st.error("Veuillez saisir votre clé API Mistral dans la barre latérale.")
        st.stop()
    ...
    async def run_mcp_query():
        ...
        # Connexion au serveur MCP distant via SSE
        async with sse_client(MCP_SERVER_URL) as streams:
            async with ClientSession(streams[0], streams[1]) as session:
                await session.initialize()
                
                # Récupération automatique des outils disponibles
                ...
                # Exécution des outils si Mistral le demande
                if message.tool_calls:
                    ...
                    # Deuxième appel à Mistral avec le résultat de l'outil
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyse avec Mistral et MCP..."):
